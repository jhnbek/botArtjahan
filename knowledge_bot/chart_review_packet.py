...
def build_kb_chart_review_packet_from_ohlc_bars(symbol: str, timeframe: str,
                                               ...
                                               *,
                                               ...
                                               breakout_direction_arg: str = "auto",
                                               manual_context: dict[str, Any] | None = None,
                                               params: ChartReviewParams | None = None,
                                               excluded_level_prices: list[float] | None = None) -> dict[str, Any]:
    ...
    excluded = [float(price) for price in (excluded_level_prices or [])]
    if excluded:
        def is_excluded(level: Level) -> bool:
            tolerance = max(abs(level.price) * 0.0005, 1e-8)
            return any(abs(level.price - price) <= tolerance for price in excluded)

        if higher_levels is not None:
            higher_levels = [level for level in higher_levels if not is_excluded(level)]
    levels = discover_levels(context_bars, level_params, higher_levels, higher_tf)
    if excluded:
        levels = [level for level in levels if not is_excluded(level)]
    return build_chart_review_packet(
        symbol,
        context_tf,
        timeframe,
...
def compact_kb_packet(packet: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "human_feedback": packet.get("human_feedback") or {},
        "layer_reports": packet.get("layer_reports") or {},
    }


def load_excluded_level_prices(exchange: str, symbol: str, interval: str) -> list[float]:
    feedback_path = ROOT / "_knowledge_base" / "user_level_feedback.jsonl"
    ...
    for raw_record in records:
        try:
            ...
            continue
        if (
            str(record.get("exchange") or "") != exchange
            or str(record.get("symbol") or "") != symbol
            or str(record.get("interval") or "") != interval
        ):
            continue
        try:
...
def load_rejected_level_feedback(exchange: str, symbol: str, interval: str) -> list[dict[str, Any]]:
    """Load active human negative labels for algorithm calibration."""
    feedback_path = ROOT / "_knowledge_base" / "user_level_feedback.jsonl"
    if not feedback_path.exists():
        return []
    rejected: dict[float, dict[str, Any]] = {}
    try:
        records = feedback_path.read_text(encoding="utf-8").splitlines()
    except OSError:
        return []
    for raw_record in records:
        ...
        if action == "hide_robot_level":
            rejected[price] = {
                ...
                "reason_codes": record.get("reason_codes") or [record.get("reason_code")],
                "reason_labels": record.get("reason_labels") or [record.get("reason_label")],
                "note": str(record.get("note") or ""),
                "recorded_at": record.get("recorded_at"),
            }
        elif action == "restore_robot_level":
            rejected.pop(price, None)
    return list(rejected.values())


def load_manual_level_prices(exchange: str, symbol: str, interval: str) -> list[float]:
    feedback_path = ROOT / "_knowledge_base" / "user_level_feedback.jsonl"
    if not feedback_path.exists():
        return []
    manual: list[float] = []
...
def build_live_kb_chart_review_packet(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        feedback_exchange = getattr(args, "feedback_exchange", "")
        feedback_symbol = str(args.instrument or args.symbol).upper()
        feedback_intervals = {str(args.timeframe)}
        context_feedback_interval = getattr(args, "context_timeframe", None)
        if context_feedback_interval:
            feedback_intervals.add(str(context_feedback_interval))
        ...
        for feedback_interval in feedback_intervals:
            for price in load_excluded_level_prices(feedback_exchange, feedback_symbol, feedback_interval):
                if price not in excluded_prices:
                    excluded_prices.append(price)
            for price in load_manual_level_prices(feedback_exchange, feedback_symbol, feedback_interval):
                if price not in manual_prices:
                    manual_prices.append(price)
            for item in load_rejected_level_feedback(feedback_exchange, feedback_symbol, feedback_interval):
                if not any(abs(float(existing["price"]) - float(item["price"])) <= 1e-8 for existing in rejected_levels):
                    item["interval"] = feedback_interval
                    rejected_levels.append(item)
        kb_packet = build_kb_chart_review_packet_from_ohlc_bars(
            ...
            manual_context={
                "drawn_levels": [{"price": price} for price in manual_prices],
            },
            excluded_level_prices=excluded_prices,
        )
        kb_packet["human_feedback"] = {
            "rejected_levels": rejected_levels,
            "rejected_level_count": len(rejected_levels),
            "manual_level_count": len(manual_prices),
        }
