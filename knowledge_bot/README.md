...
Подключение к базе из `botArtjahan-main` уже реализовано в
[`knowledge_base.py`](knowledge_base.py). См. [команды поиска и обновления](KNOWLEDGE_CONNECTION.md).
Этот модуль работает независимо от остальных частей конвейера.
Исходники конвейера восстановлены из локальной истории Git; их перечень —
в `../RESTORATION_FILES.txt`. Локальные данные конвейера восстанавливаются отдельно.
...
### Калибровка уровней по ручной проверке

Удалённые уровни робота записываются как отрицательные примеры (`human_rejected`),
а добавленные вручную — как положительные (`human_accepted`). Сводный профиль и
отчёт пересобираются после каждой записи из desktop UI. Их можно также построить
вручную:

```powershell
\.\.venv\Scripts\python.exe -X utf8 .\knowledge_bot\build_level_feedback_statistics.py
```

Результаты сохраняются в `_knowledge_base/level_feedback_statistics.json` и
`_knowledge_base/level_feedback_statistics.md`. Пороговые правила не меняются
автоматически: профиль предназначен для проверяемой калибровки алгоритма.
